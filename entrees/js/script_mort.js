...
/*function triggerAutoCompleteFarm(event) {
    $('#deathFarm').autocomplete({
        source: "../../libraries/ajax/suggestFarm.php?",
        dataType: "json",
        select: function(event,ui){
            event.preventDefault();
            $('#deathFarm').val(ui.item.nom_elevage);
    ...
    if (event.which!=13 && event.which!=9 && event.which!=16){
        if (document.getElementById('deathFarm').value == ''){
...
function triggerAutocompleteAnimal(event) {
    ...
    var target2 = $('#deathFarm');
    var target3 = $('#farmId');
    animalID.autocomplete({
        source: "../../libraries/ajax/suggestAnimalAndFarm.php?ajaxType=1&race="+Race,
        dataType: "json",
        select: function (event,ui){
            event.preventDefault();
    ...
    if (event.which!==13 && event.which!==9 && event.which!==16){
        ...
        document.getElementById('farmId').value='';
        document.getElementById('deathFarm').value='';
    }
}
